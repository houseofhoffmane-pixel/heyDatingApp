  ...
  async profileDetail(viewerId: string, targetUserId: string) {
    ...
    if (myLoc && targetProfile) {
      const rows = await this.prisma.$queryRaw<{ d: number }[]>`
        SELECT ST_Distance(location, ST_SetSRID(ST_MakePoint(${myLoc.lng}, ${myLoc.lat}), 4326)::geography)::float AS d
        FROM "profiles" WHERE "id" = ${targetProfile.id}::uuid
      `;
  ...
  private async fetchCandidates(viewerId: string): Promise<CandidateRow[]> {
    return this.prisma.$queryRaw<CandidateRow[]>`
      ...
      SELECT
        ...
        p.relationship_intent::text AS relationship_intent,
        p.drinks::text        AS drinks,
        p.smokes::text        AS smokes,
        p.exercise::text      AS exercise,
        p.weed_420::text      AS weed_420,
        p.kids::text          AS kids,
        p.politics::text      AS politics,
        p.religion::text      AS religion,
        p.monogamy::text      AS monogamy,
        p.star_sign::text     AS star_sign,
        ST_Distance(p.location, (SELECT my_location FROM viewer))::float AS dist_m,
        EXISTS (
          SELECT 1 FROM likes l
          WHERE l.from_user_id = u.id AND l.to_user_id = ${viewerId}::uuid
        ) AS they_liked_me,
        (SELECT array_agg(pi.interest_id::text) FROM profile_interests pi WHERE pi.profile_id = p.id) AS interest_ids
      FROM profiles p
      ...
      WHERE u.status = 'active'
        ...
        AND ST_DWithin(p.location, viewer.my_location, viewer.distance_mi * 1609.34)
        AND u.dob IS NOT NULL
        AND EXTRACT(YEAR FROM AGE(u.dob)) BETWEEN viewer.age_min AND viewer.age_max
        ...
        AND (
          CASE p.gender
            ...
            ELSE 'non-binary'
          END = ANY(viewer.my_lf)
          OR 'everyone' = ANY(viewer.my_lf)
        )
        AND (
          CASE viewer.my_gender
            ...
            WHEN 'trans_man'   THEN 'men'
            ELSE 'non-binary'
          END = ANY(p.looking_for)
          OR 'everyone' = ANY(p.looking_for)
        )
        AND NOT EXISTS (
        ...
        AND NOT EXISTS (SELECT 1 FROM likes  l  WHERE l.from_user_id  = ${viewerId}::uuid AND l.to_user_id  = u.id)
        AND NOT EXISTS (SELECT 1 FROM passes ps WHERE ps.from_user_id = ${viewerId}::uuid AND ps.to_user_id = u.id)
        AND (
